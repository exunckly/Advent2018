...
# Bear in mind that we might run into problems if R knows about leap years and daylight savings time?!
...
d4t <- as.tibble(d4) %>%
  ...
  mutate(jumble = gsub(" ","_",jumble)) %>%                              # Prepare tidy coliumn names
...
# Funtion that returns a 60-length vector with the kength of time  aguard was asleep during each minute
# Note that munite 0 is held in location 1 and so on, so subtract 1 from the index at the very end to get the actual minute
...
part1_ans <- sleepiest_g$guard_no[1]* part1_best_times
...
my_loc <- which(gp2_sleeps == max(gp2_sleeps), arr.ind = TRUE)

part2_ans <- guards[my_loc[1]] * (my_loc[2] - 1)
